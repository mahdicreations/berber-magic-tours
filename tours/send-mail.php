...
$tour_name = 'Morocco Tour Booking';
...
if (empty($name)) $name = 'Guest Traveler';
if (empty($email)) $email = 'info@berber-magic-tours.com';

$subject = "Tour Booking Request: " . $tour_name . " - " . $name;
...
$body_html = '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0; padding:0; background-color:#F4F6F8; font-family:\'Segoe UI\', Tahoma, Geneva, Verdana, sans-serif; color:#333;">'
           ...
           . '<p style="color:#D95D39; margin:6px 0 0 0; font-size:13px; font-weight:600; text-transform:uppercase; letter-spacing:1.5px;">Tour Booking Request</p>'
           . '</td></tr>'
           . '<tr><td style="padding: 30px;">'
           . '<h2 style="color:#1E293B; font-size:18px; margin-top:0; margin-bottom:15px; border-bottom:2px solid #F1F5F9; padding-bottom:10px;">' . htmlspecialchars($tour_name) . '</h2>'
           . '<table border="0" cellpadding="8" cellspacing="0" width="100%" style="border-collapse:collapse; margin-bottom:20px;">'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td width="38%" style="font-weight:600; color:#1E293B; padding:10px 0;">Selected Tour:</td><td style="color:#475569; padding:10px 0;"><strong>' . htmlspecialchars($tour_name) . '</strong></td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Full Name:</td><td style="color:#475569; padding:10px 0;">' . htmlspecialchars($name) . '</td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Email Address:</td><td style="color:#475569; padding:10px 0;">' . htmlspecialchars($email) . '</td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">WhatsApp / Phone:</td><td style="color:#475569; padding:10px 0;">' . htmlspecialchars($phone) . '</td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Travel Date:</td><td style="color:#475569; padding:10px 0;">' . htmlspecialchars($date) . '</td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Travelers Count:</td><td style="color:#475569; padding:10px 0;">' . htmlspecialchars($travelers) . ' Person(s)</td></tr>'
           . '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Travel Style:</td><td style="color:#D95D39; font-weight:700; padding:10px 0;">' . htmlspecialchars($style) . '</td></tr>'
           . (!empty($message) ? '<tr style="border-bottom:1px solid #F1F5F9;"><td style="font-weight:600; color:#1E293B; padding:10px 0;">Special Requests:</td><td style="color:#475569; padding:10px 0;">' . nl2br(htmlspecialchars($message)) . '</td></tr>' : '')
           . '</table>'
...
$sent = false;
foreach ($hosts as $h) {
    if (send_smtp($h['host'], $h['port'], $smtp_user, $smtp_pass, $smtp_user, $to_email, $subject, $body_html, $email)) {
        $sent = true;
        break;
    }
}
...
echo json_encode([
    'status' => 'success',
    'message' => 'Thank you! Your reservation request for ' . htmlspecialchars($tour_name) . ' has been submitted successfully. Our team will contact you shortly.'
]);
