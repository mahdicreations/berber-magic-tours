...
print(f"Collected metadata for {len(tour_meta)} tours.")
...
# Generate custom recommended section for each tour
print("\n=== GENERATING CATEGORY-RELATED RECOMMENDED TOURS FOR EACH TOUR PAGE ===")
updated_count = 0
...
for filename, meta in tour_meta.items():
    ...
    # If same category has fewer than 3, pick related fallback tours from other categories
    related_tours = same_cat_tours[:]
    if len(related_tours) < 3:
        # Fallback priority categories
        all_other = [t for t, m in tour_meta.items() if t != filename and t not in related_tours]
        related_tours.extend(all_other[:(3 - len(related_tours))])

    # Select top 3 related tours
    selected_3 = related_tours[:3]

    # Generate HTML cards for selected 3
    cards_html = ""
    ...
    <section class="recommended-tours-section" style="padding: 50px 0; background: #FAFAFA; border-top: 1px solid #E2E8F0;">
        <div class="container">
            <div style="margin-bottom: 30px; text-align: center;">
                ...
                <p style="color: #64748B; font-size: 0.92rem; margin: 0;">Explore similar {curr_cat.lower()} and popular itineraries crafted by Berber Magic Tours.</p>
            </div>
...
{cards_html}
    ...
    if re.search(pattern, file_content):
        ...
        updated_count += 1
        print(f"Updated category-related Recommended Tours in [{filename}] (Category: {curr_cat})")

print(f"\nCompleted! Dynamically generated category-related Recommended Tours across {updated_count} tour pages.")
